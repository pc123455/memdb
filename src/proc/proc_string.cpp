...
int proc_get(Client* client, DbEngine* db, const Request & request, Response & response) {
    std::string val;
...
int proc_set(Client* client, DbEngine* db, const Request & request, Response & response) {
    std::string resp;
...
int proc_getrange(Client* client, DbEngine* db, const Request & request, Response & response) {
    std::string val;
...
int proc_getset(Client* client, DbEngine* db, const Request & request, Response & response) {
    std::string val;
    val = request[2];
...
int proc_mget(Client* client, DbEngine* db, const Request & request, Response & response) {
    int result = db->mget(request, response);
    return result == DbEngine::DB_OK ? Proc::PROCESS_OK : Proc::PROCESS_ERROR_MSG;
}

int proc_setnx(Client* client, DbEngine* db, const Request & request, Response & response) {
